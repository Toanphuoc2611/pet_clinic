...
class KennelPdfPreviewScreen extends StatefulWidget {
  final InvoiceKennelDto invoice;

  const KennelPdfPreviewScreen({super.key, required this.invoice});

  @override
  State<KennelPdfPreviewScreen> createState() => _KennelPdfPreviewScreenState();
}

class _KennelPdfPreviewScreenState extends State<KennelPdfPreviewScreen> {
  ...
  Future<void> _generatePdf() async {
    try {
      ...
      final pdfData = await PdfService.generateKennelInvoicePdf(widget.invoice);
    ...
    if (_pdfData != null) {
      try {
        await Printing.layoutPdf(
          ...
          name: 'Hoa_don_chuong_${widget.invoice.invoiceCode}.pdf',
        );
    ...
    if (_pdfData != null) {
      try {
        await Printing.sharePdf(
          bytes: _pdfData!,
          filename: 'Hoa_don_chuong_${widget.invoice.invoiceCode}.pdf',
        );
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Widget _buildBody() {
    ...
    if (_pdfData != null) {
      return PdfPreview(
        ...
        canDebug: false,
        maxPageWidth: 700,
        pdfFileName: 'Hoa_don_chuong_${widget.invoice.invoiceCode}.pdf',
      );
    }
